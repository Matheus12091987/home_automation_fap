...
class EdicaoDevices extends StatefulWidget {
  final int passaIndex;
  final int passaId;

  EdicaoDevices({Key key, @required this.passaIndex, @required this.passaId})
      : super(key: key);
  @override
  _EdicaoDevicesState createState() => _EdicaoDevicesState(
    indexLocalPassado: passaIndex,
    idDeviceSelecionado: passaId,
  );
}

class _EdicaoDevicesState extends State<EdicaoDevices> {
  final int indexLocalPassado;
  final int idDeviceSelecionado;

  _EdicaoDevicesState(
      {@required this.indexLocalPassado, @required this.idDeviceSelecionado});
  ...
  void initState() {
    _txtDeviceCadastro.text = devicesName[indexLocalPassado];
    _txtComentarioCadastro.text = devicesComentario[indexLocalPassado];
    _radioValue = devicesTipo[indexLocalPassado] - 1;
    valorRadio = devicesTipo[indexLocalPassado];
    _txtAddrLocalCadastro.text = devicesAddrLocal[indexLocalPassado];
    _txtAddrRemotoCadastro.text = devicesAddrRemoto[indexLocalPassado];
    _txtFeedbackCadastro.text = devicesFeedback[indexLocalPassado];
  }

  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Atualização/Edição de Devices'),
    ...
    if (_currentIndex == 0) {
      ...
      if (_txtDeviceCadastro.text.isNotEmpty &
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text('Device Atualiado com Sucesso!!!'),
          duration: Duration(seconds: 2),
        ));
      } else {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content:
              Text('!!!Para Atualizar o Device não pode haver campos nulos!!!'),
